...
function AboutPage({ developerEnabled, testPushBusy, testPushMessage, testEmailBusy, testEmailMessage, emailRemindersEnabled, onClose, onFaq, onToggleDeveloper, onTestPush, onTestEmail }) {
  ...
  return (
    <section className="info-page" aria-labelledby="about-title">
      ...
      <div className="info-content">
        ...
        {developerEnabled && <section className="developer-panel"><strong>🛠 Developer Mode</strong><p>Verify push and email delivery through callable Cloud Functions.</p><div className="developer-actions"><button type="button" disabled={testPushBusy} onClick={onTestPush}>{testPushBusy ? "Sending…" : "Send Test Push"}</button><button type="button" disabled={testEmailBusy || !emailRemindersEnabled} onClick={onTestEmail}>{testEmailBusy ? "Sending…" : "Send Test Email"}</button></div>{!emailRemindersEnabled && <small>Enable Email Reminders in Settings to test email delivery.</small>}{testPushMessage && <div className={`form-message ${testPushMessage.type}`}>{testPushMessage.text}</div>}{testEmailMessage && <div className={`form-message ${testEmailMessage.type}`}>{testEmailMessage.text}</div>}</section>}
